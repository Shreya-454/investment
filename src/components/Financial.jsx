...
const Financial = () => {
    const datacard1 = data1.map((card,i) =>(
        ...
        <div className={`w-full max-lg:max-w-[558px] rounded-3xl bg-[#110B26] before:top-0 before:h-full before:bottom-0 group before:bg-[#AB68FF] before:!duration-500 before:left-0 before:right-[200%] hover:before:right-0  before:z-[-1] before:absolute border-2 border-solid z-[1] after:z-[2] h-[450px] md:h-[230px] border-[#AB68FF4D] p-[29px_20px_29.44px_0px] after:w-[12px] overflow-hidden border-l-[#AB68FF]  after:bg-[#AB68FF] after:absolute relative after:left-[-2px] after:top-0 after:bottom-0 ${card.class}`} data-aos="zoom-in">
            <div className="sm:flex justify-between items-center">
                <div className='pl-7'>
                    <h4 className=' font-Inter text-white text-xl max-sm:text-lg sm:leading-[24.2px] font-semibold pb-[14px] max-sm:text-center'>{card.head}</h4>
                    <p className={`font-Inter max-sm:text-sm text-base text-white font-normal max-w-[259px] max-sm:mx-auto opacity-70 max-sm:text-center ${card.style}`}>{card.para}</p>
                </div>
  ...
  return (
    <div className=' bg-[url(./assets/images/webp/financialbg.webp)] bg-fixed bg-cover bg-center bg-no-repeat md:pt-[200px] md:pb-[154px] sm:py-[100px] py-10 relative overflow-x-clip z-[2]'>
      ...
      <div className=' container max-w-[1164px] 2xl:max-w-[1320px] mx-auto px-3'>
          <h2 className=' font-inter text-center text-white max-sm:text-3xl text-[40px] sm:leading-[48.41px] font-bold pb-[60px]'>Your Gateway to <span className=' font-extrabold text-[#AB68FF]'>Financial Empowerment</span></h2>
